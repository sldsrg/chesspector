...
export class Queen extends Piece {
  constructor( row: number, column: number, isWhite: boolean) {
    super(row, column, isWhite, isWhite ? 'Q' : 'q')
  }
  ...
  public getPseudoLegalMove(
    ...
    if (Math.abs(this.square.column - toColumn) !== Math.abs(this.square.row - toRow) &&
      this.square.column !== toColumn && this.square.row !== toRow) return
